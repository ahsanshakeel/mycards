...
namespace MyCard.Web.Models
{
    public class GraphService
    {
        ...
        // Get the current user's DisplayName address from their profile.
        public async Task<string> GetDisplayName(GraphServiceClient graphClient)
        {

            // Get the current user. 
            // This sample only needs the user's email address, so select the mail and userPrincipalName properties.
            // If the mail property isn't defined, userPrincipalName should map to the email for all account types. 
            User me = await graphClient.Me.Request().Select("displayName,userPrincipalName").GetAsync();
            return me.DisplayName ?? me.UserPrincipalName;
        }

        // Get the current user's email address from their profile.
        public async Task<User> GetUserInfo(GraphServiceClient graphClient)
        {
